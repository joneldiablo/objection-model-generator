...
module.exports = async function (dbName, dbConnection, modelsPath, controllerParent, pathControllers) {
  var DB = dbName;
  // Initialize knex.
  var knex = Knex({
    client: 'mysql',
  ...
  var check = await knex.raw('select 1+1 as result').catch(function (e) {
    return { error: e };
  });
  if (check.error) {
  ...
  var pathHeader = path.join(__dirname, 'templates/controllerHeaderTemplate.mustache');
  var templateControllerHeader = fs.readFileSync(pathHeader, 'UTF-8');
  var pathControllerTpl = path.join(__dirname, 'templates/controllerTemplate.mustache');
  var templateController = fs.readFileSync(pathControllerTpl, 'UTF-8');
  var headerController = Mustache.render(templateControllerHeader, {
  ...
  tables.forEach(function (table) {
    ...
    var rels = table.columns.filter(function (col) {
      return !!col.constrain;
    }).map(function (col) {
      var targetTableName = singularize(col.constrain.REFERENCED_TABLE_NAME);
